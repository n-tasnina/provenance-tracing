...
def setup_opts():
    ...
    group.add_argument('--path_type_comp', action='store_true', default=True,
                       help="Compute contribution from paths of different length and type")
...
def compute_and_save_contr_from_different_types_of_paths(M_pathmtx, R, pos_nodes_idx,
                top_k_pred_idx, paths_stat_file, force_run=False):
    ...
    if (not os.path.isfile(paths_stat_file)) or force_run:
        ...
        frac_contr_from_different_paths_df.to_csv(paths_stat_file, sep='\t',index=False)
        ...
        print('Contribution from different path types is already present')

    frac_contr_from_different_paths_df = pd.read_csv(paths_stat_file, sep='\t',index_col=None)
    return frac_contr_from_different_paths_df
...
def main(config_map, k, **kwargs):
    ...
    *kwargs*: all of the options passed into the script
    ...
    viz_dir = config_map['output_settings']['output_dir']+"/viz/"
    os.makedirs(viz_dir,exist_ok=True)
    rank_match_file = viz_dir + "rank_match.tsv"
    with open(rank_match_file, 'w') as f:
        f.write("network\talg_name\tRBO_all\tRBO_top1000\n")
    f.close()
    ...
    for dataset in input_settings['datasets']:
        network_name = dataset['net_version']
        print("Loading data for %s" % (network_name))
            ...
            for alg_name in alg_settings:
                if (alg_settings[alg_name]['should_run'][0] == True) or (alg_name in kwargs.get('run_algs')):
                    ...
                    for alpha, alg in zip(alphas, alg_pred_files):
                        ...
                        #keep the top k predictions only
                        df_top = df[:k]
                        top_k_pred = df_top['prot']
                        print(df_top, '\n\n\n')
                        # top_k_pred_idx = [net_obj.node2idx[n] for n in top_k_pred]
                        top_k_pred_idx = df_top['prot_idx'].tolist()
                        pred_scores[top_k_pred_idx] = df_top['score'].values
                        ...
                        if kwargs.get('path_type_comp'):
                            path_type_based_contr_df = compute_and_save_contr_from_different_types_of_paths \
                                (M_pathmtx, R, pos_nodes_idx, top_k_pred_idx, paths_of_different_types_stat_file, force_run=False)
                            rbo_overlap_all, rbo_overlap_top_1000  = script_utils.track_rank(path_type_based_contr_df, df)

                            with open(rank_match_file, 'a') as f:
                                f.write(f'{network_name}\t{alg_name}\t{rbo_overlap_all}\t{rbo_overlap_top_1000}\n')
                        ...
                        if (not os.path.exists(nsp_processed_paths_file)) or (kwargs.get('force_contr') == True):
                            ...
                            eppstein_code_dir = f'{project_root}/k-shortest-paths/'
