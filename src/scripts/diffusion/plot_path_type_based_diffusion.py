...
def boxplot_diffusion_along_path_types(diffusion_path_types,title, top_or_bottom, term, param_str, plot_dir,
    ...
    df_new=df.rename(columns={'frac_pathlen_1':'1','frac_pathlen_2':'2','frac_pathlen_3':'3','frac_pathlen_4':'4','frac_beyond_pathlen4':'>4'})
    plt.figure(figsize=(3,4))

    from matplotlib.colors import ListedColormap
    # colors = sns.color_palette("ch:start=.2,rot=-.3")
    # colors = sns.cubehelix_palette(start=.5, rot=-.5)
    # colors = sns.cubehelix_palette(start=.5, rot=-.75, light=0.85, dark=0.2)
    colors = sns.color_palette("viridis_r")
    sns.boxplot(data = df_new.set_index('target'), palette =colors, linewidth=0.3)

    plt.ylim([0, 1])
    plt.xlabel('Path lengths', fontsize=13)
    plt.ylabel('Fraction of score', fontsize=13)
        ...
        for alg_name in alg_settings:
            ...
            if (alg_settings[alg_name]['should_run'][0] == True) or (alg_name in kwargs.get('run_algs')):
                ...
                for term in ann_obj.terms:
                    ...
                    ed_file = config_map['output_settings'][
                                                   'output_dir'] + "/viz/%s/%s/diffusion-analysis/%s/effective-diff-k%s-a%s%s.tsv" \
                                               % (dataset['net_version'], term, alg_name, k, balancing_alpha, sig_str)
