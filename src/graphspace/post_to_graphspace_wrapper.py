...
EDGE_DIR_MAP = {
        ...
        'miscellaneous': True,
        'up-regulation': True,
        'down-regulation': True,
        'unknown': True,
}

EDGE_COLORS_INTERACTION_TYPE = {
    'physical': '#ebbd34',  # orange
    'activation': 'green',
    'inhibition': 'red',
    'phosphorylation': 'black',
    'enzymatic': 'brown',
    'binding': '#34bdeb', #light blue
    # 'dir_predicted': '#0d2859', #navy blue
    'miscellaneous' : 'grey',
}

EDGE_COLORS_EFFECT_TYPE = {
    'up-regulation': '#00ff00', #lime
    'down-regulation': 'red',
    'unknown' : 'grey',
}

ARROW_SHAPE_EFFECT_TYPE = {
'up-regulation': 'triangle',
'down-regulation': 'tee',
'unknown' : 'none',
}
# order to use when deciding which color an edge will get
edge_type_order = ['activation', 'inhibition', 'phosphorylation', 'binding',  'enzymatic', 'physical',
                   'dir_predicted', 'miscellaneous']
...
def call_post_to_graphspace(prededges, sources, targets, top_targets, uniprot_to_gene,
                            uniprot_to_protein_names,out_file,
                            selected_terms=None, term_names=None, term_prots=None,
                            term_pvals=None, causal_edges=None, **kwargs):

    init_node_legend_dict() #fresh node_legend_dict to remove any lingering legend

    if selected_terms!=None:
...
def build_graph_and_post(prededges, sources, targets,top_targets, uniprot_to_gene,uniprot_to_protein_names,
        causal_edges=None, graph_attr_file=None, ev_file=None, **kwargs):
    ...
    G = constructGraph(
        ...
        graph_attr=graph_attr, attr_desc=attr_desc,causal_edges=causal_edges, **kwargs)
    ...
    layout = gs_utils.grid_layout(G, graph_attr, sort_by='source_2_target')
...
def constructGraph(
        ...
        graph_attr={}, attr_desc={}, causal_edges=None, **kwargs):
    '''
    ...
    for (u,v) in prededges:
        ...
        if edge_types: #if we have type information for edges
            # main_edge_type = getMainEdgeInteractionType(edge_types[(u, v)])
            #considering effect type i.e., up-regulation /down-regulation
            main_edge_type = edge_types[(u, v)]
        else:
            main_edge_type=None

        if main_edge_type is None:
            # TODO: how to set edge type
            main_edge_type = "miscellaneous"
        ...
        # color = EDGE_COLORS_EFFECT_TYPE[main_edge_type]

        #generic edge style
        color = 'red'
        edge_style = 'solid'
        width = 1.5
        arrow_shape = ARROW_SHAPE_EFFECT_TYPE[main_edge_type]


        if causal_edges!=None:
            if (u, v) in causal_edges: #override edge color with 'blue' if causal edge
                color = '#0000ff' #blue

        edge_str = "%s-%s" % (u,v)
        # override edge attributes if mentioned in graph_attr
        ...
        G.add_edge_style(gene_name_u, gene_name_v, attr_dict=attr_dict,
                         directed=EDGE_DIR_MAP[main_edge_type], color=color,
                         width=width, arrow_shape=arrow_shape,edge_style=edge_style)
    return G
...
def getMainEdgeInteractionType(edge_types):
